...
inAccess (Lex acc _, ths) = case acc of
    VariableAccess idenL       -> Nothing
    ArrayAccess    accL indexL -> Just (accL, HistoryArray  indexL : ths)
    StructAccess   accL fieldL -> Just (accL, HistoryStruct fieldL : ths)

outAccess :: Zipper -> Maybe Zipper
outAccess (accL, ths) = case ths of
    []                        -> Nothing
    HistoryArray  indexL : ts -> Just (ArrayAccess  accL indexL <$ accL, ts)
    HistoryStruct fieldL : ts -> Just (StructAccess accL fieldL <$ accL, ts)

topAccess :: Zipper -> Maybe Zipper
topAccess z@(accL, ths) = case ths of
    []     -> Nothing
    t : ts -> topAccess $ fromJust $ outAccess z

deepAccess :: Zipper -> Maybe Zipper
deepAccess z@(Lex acc _, ths) = case acc of
    VariableAccess _ -> Just z
